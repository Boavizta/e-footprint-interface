...
The dashboard is **re-rendered fresh on every visit** (no cached comparison) so an edit to either model always shows up; it is loaded into `#main-content-block` via HTMX and **deliberately does not `hx-push-url`**. It is an ephemeral projection of two models, not a navigable resource (no model slot gets its own URL either — the active slot is server-side state), and the `compare` view skips the `drop_expired_slots` + recovery fallback that `model_builder_main` has, so a refresh of a pushed `/compare/` would 500 once a slot's cache TTL lapses. Leaving the URL at `/model_builder/` means a refresh always lands on the self-healing builder, Compare one click away. The dashboard is a **self-contained view**: it carries the shared tab strip but **no per-model chrome** — no toolbar (tools / system name), no floating "Show results" button, no side-panel/help-drawer/result containers — so the `compare` view needs no `model_web` in context (just the comparison view model, `workspace_slots`, and `active_slot`). From the dashboard a **model tab returns to the builder** (`model_comparison.js#switchToSlot` detects the absence of resident canvases and re-opens `/model_builder/`, which restores the builder chrome). Because there is no chrome to rebind, the dashboard's tab strip renders with `on_dashboard=True`, which makes its switch POST carry `skip_chrome`; `switch_model` then emits only the `switchModelCanvas` trigger (no shared-chrome OOB, which would otherwise hit missing targets → `oobErrorNoTarget`).
