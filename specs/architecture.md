...
**Tabs are fixed role labels; the model name lives in the toolbar.** Each model tab shows a fixed label by *list position* — first slot "Reference model", second "Comparison model" (matching how `compare()` pairs `workspace_slots[0]→A`, `[1]→B`) — not the model's own name, so a long name can never stretch a tab (the tab width is pinned in `custom.scss`). The active model's editable name lives in the toolbar (`#btn-change-system-name` / `#system-name`, left of the edge toggle); renaming swaps `#system-name` in place and switching OOB-rebinds it (above).
