...
- **Workspace file (`.e-fw.json`).** An *additive* envelope `{ efootprint_workspace_version, active_slot, models: [<doc>, <doc>] }` where each `models[]` element is a **byte-for-byte single-model document** (the same `download_json` payload, no calculated attributes) — so the single-model format is never re-implemented or altered. `download_workspace`/`upload_workspace` (`views.py`/`views_workspace.py`) produce/consume it; `_single_model_document(repository)` is the shared per-slot builder. **Detection is content-based** (authoritative): a top-level `models` key ⇒ workspace file, its absence ⇒ single-model file; the `.e-fw.json` extension is a UX hint only. On import each model is recomputed (`ProgressiveImportService`) and the second is added through `add_slot`, so the shared budget *and* the distinct-system-id invariant both apply (the two embedded models may legitimately share an id). **Cross-format robustness** (honest-error): "Open workspace file" fed a single-model file loads it into the active slot; `upload_json` ("Replace this model") fed a workspace file rejects it with guidance *before* any mutation. The two-granularity Download/Upload menu (this model / whole workspace) renders only when the workspace holds two models — a single-model session keeps today's plain Import/Export toolbar (and its E2E selectors) unchanged.
