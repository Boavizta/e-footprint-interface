...
- **Confidence cell** — renders the same `confidence_badge.html` used in the side panel. Picking a level autosaves immediately and refreshes `#source-block`. Pure event-driven (no Apply button).
...
- `model_builder/templates/model_builder/side_panels/dynamic_form_fields/confidence_badge.html` — gained optional `conf_autosave_url` / `conf_autosave_refresh_url` context. When set, emits `data-autosave-*` data attrs on the wrap.
- `theme/static/scripts/source_metadata.js` —
  - `setConfidence` branches on `wrap.dataset.autosaveUrl`; `autosaveConfidence` POSTs the single field and refreshes `#source-block`.
...
- **Source table — confidence cell:** open the result panel, switch to the Sources tab, click the inline badge in any input row, pick a level. Verify the table refreshes with the new badge state; reload the page and verify it persists.
- **Source table — pencil affordance:** click the pencil on an input row. Verify the row expands to show the source editor (no nested editor layers, single Cancel/Apply). Pick a different source and add a comment, click Apply; verify the table refreshes and the row shows the new source/comment.
