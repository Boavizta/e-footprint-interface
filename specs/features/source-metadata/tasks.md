...
## Task 2 — Library: `confidence` + `comment` on `ExplainableObject` (v21.1.0)
...
- `pyproject.toml` (library) — bump `version` to `21.1.0`.
- `CHANGELOG.md` — minor-version entry.
...
- Loading a v21.0.0 JSON without these fields succeeds with both attrs set to `None`.
...
- `model_builder/domain/object_factory.py` — new `_apply_metadata(explainable_object, parsed_meta, value_changed, model_web)` helper. Used by both `create_efootprint_obj_from_parsed_data` and `edit_object_from_parsed_data`. Rules:
  - resolve submitted source against `model_web.available_sources` by id; if id missing or unknown, mint a new `Source(name, link)`;
  - if `value_changed`: force `confidence = None`; carry `comment` through unless the user explicitly cleared it.
  - replaces the existing default-source-on-value-change line in `edit_object_from_parsed_data` (line 174-ish).
...
- `theme/static/scripts/source_metadata.js` (new, ~80 lines vanilla JS) — confidence dropdown open/close + value selection; source-editor open/close + Apply/Cancel; custom-name collision detection against `available_sources` names.
- `theme/static/scripts/dynamic_forms.js` — `updateSource` no longer rewrites the source div. On input value change: pulse the badge with a `confidence-just-reset` animation and clear the hidden `__confidence` input. The actual reset is enforced by Task 4's factory rule.
