...
- `model_builder/templates/model_builder/components/model_canvas_content.html` — wrap the four edge top-level add-buttons (`add_edge_usage_pattern`, `btn-add-edge-usage-journey`, `btn-add-edge-device`, `btn-add-edge-device-group`) in `<div data-modeling-paradigm="edge">` so the body-class CSS hides them when edge modeling is off. The attribute is only load-bearing here: latching makes any other placement (cards, dot) functionally unobservable.
- `model_builder/templates/model_builder/components/button_card_header.html` — render the `.modeling-paradigm-dot` popover span when `object_card.modeling_paradigm == "edge"` (markup per plan §"Card badge"). No `data-modeling-paradigm` attribute is needed; the dot already disambiguates edge cards visually, and latching prevents the off-state from ever co-existing with edge cards.
