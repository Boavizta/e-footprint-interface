...
- **Form-less inline edits go through `data-autosave-*` on the host element.** When a control needs to persist on click without an enclosing form (e.g. the confidence badge in the source table), include the partial with `conf_autosave_url` + `conf_autosave_refresh_url`; the JS reads `data-autosave-*` on the wrap, POSTs only the changed field, then refreshes the host region. `_apply_metadata` patches in place, so omitted source/comment/confidence keys are preserved. See `confidence_badge.html` + `source_metadata.js#autosaveConfidence`.
