...
- **Pick the persistence shape that fits the trigger.** For event-driven, no-button widgets (e.g. the confidence badge — a menu pick *is* the save), use the `data-autosave-*` convention: include the partial with `*_autosave_url` + `*_autosave_refresh_url`, JS POSTs the changed field, then refreshes the host region. For Apply-button widgets (e.g. the source editor in the source-table row), wrap them in a real HTMX form (`hx-post hx-swap="none" data-action="source-table-row-edit"`) — Apply writes hidden inputs via the existing `applySourceEditor` and triggers the form's submit, so there's no parallel "autosave" code path. `_apply_metadata` patches in place either way, so omitted source/comment/confidence keys are preserved.
