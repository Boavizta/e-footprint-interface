...
### Changed
- Recurrent timeseries fields now keep their standard field label aligned with ordinary inputs, with the compact type selector and decorated subform nested beneath it. Weekly patterns use compact summaries, profile controls, and day pills, with a full-size live preview fixed immediately left of the side panel (inline on phones); forms with several weekly fields show only the preview currently being edited. Weekly and hourly previews share the same panel template, typography, spacing, shadow, and desktop geometry, and successful previews stay silent while refresh and error states remain visible.
...
### Fixed
- Upgraded to e-footprint 21.1.1 to fix a bug that happened when opening the result panel on an edge modeling without server jobs.

### Added
- Better testing with results view-layer smoke test across model archetypes.
...
### Changed
- Centralised Bootstrap widget initialization in `theme/static/scripts/bootstrap_widgets.js` (new `initBootstrapWidgets(root)` wired to `DOMContentLoaded` and `htmx:afterSettle`, scoped to the swap target). Removed per-module popover init from `edge_modeling_toggle.js` and `help_drawer_utils.js`, and removed the `initDisabledButtonTooltips` helper from `model_builder_main.js`. The pre-existing gap where disabled-button tooltips weren't re-attached after HTMX swaps is fixed as a side effect. `.truncated-text-tooltip` keeps its dedicated init (truncation-guard + zero delay).
...
### Added
- Sankey diagrams for impact repartition breakdowns.
...
### Fixed
- Result graph js files hashing to avoid caching issues.

## [0.14.3] - 2026-02-24

### Fixed
- Added External APIs to results categories in the results panel.

### Added
- External API server calculated attributes in External API calculated attributes section. 
