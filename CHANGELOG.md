...
### Changed
- Recurrent timeseries fields now group their definition selector, active subform, and source beneath a stronger variable heading. Weekly patterns use compact summaries, profile controls, and day pills, with a full-size live preview fixed immediately left of the side panel (inline on phones); forms with several weekly fields show only the preview currently being edited.
