...
- `edge_modeling_toggle.js`: wrapped in an IIFE (no more leaked top-level names), switched change-event dispatch from element id to `data-action="edge-modeling-toggle"` (per `conventions.md → JavaScript`), and scoped popover initialization to the HTMX swap target instead of querying the whole document on every settle.
