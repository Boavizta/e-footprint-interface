...
from efootprint.abstract_modeling_classes.source_objects import SourceRecurringValues, SourceValue
...
class RecurrentEdgeProcessFromForm(RecurrentEdgeProcess):
    ...
    def __init__(
        ...
        super().__init__(
            name,
            recurrent_compute_needed=SourceRecurringValues(Quantity(np.array([0] * 168, dtype=np.float32), u.cpu_core)),
            recurrent_ram_needed=SourceRecurringValues(Quantity(np.array([0] * 168, dtype=np.float32), u.GB)),
    ...
    def update_recurrent_compute_needed(self):
        recurrent_compute_needed = ExplainableRecurringQuantities(
