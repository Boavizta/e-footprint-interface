...
def open_edit_object_panel(request, object_id):
    ...
    object_belongs_to_computable_system = (
        len(model_web.system.servers) > 0
        or len(model_web.system.external_apis) > 0
        or len(model_web.system.edge_devices) > 0
    ) and (len(obj_to_edit.systems) > 0)
    context_data["object_belongs_to_computable_system"] = object_belongs_to_computable_system

    http_response = render(request, f"model_builder/side_panels/edit/{obj_to_edit.edit_template}", context=context_data)
...
def open_link_existing_panel(request, parent_id, child_type_str):
    ...
    attr_name = next(
        attr for attr, type_str in parent_obj.child_attr_names_to_child_types_str.items() if type_str == child_type_str
    )

    if attr_name in parent_obj.dict_attr_names:
        field = build_dict_count_field_from_annotation(
            attr_name,
            parent_obj.class_as_simple_str,
            child_type_str,
            {attr_name: parent_obj.get_efootprint_value(attr_name)},
            model_web,
            obj_to_edit=parent_obj,
        )
    else:
...
def open_panel_system_name(request):
    ...
    return render(
        request,
        "model_builder/side_panels/rename_system.html",
        context={
