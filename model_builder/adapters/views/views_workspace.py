...
def open_upload_workspace_panel(request):
    """Side panel for "Open workspace file" — restores both models from one ``.e-fw.json`` file."""
    return render(request, "model_builder/side_panels/upload_workspace.html", context={
        "header_name": "Open a workspace file", "save_button_label": "Open workspace"})


@require_POST
@time_it
def upload_workspace(request):
    """Restore a workspace from an uploaded file (model-comparison §2.7).

    Content-based detection (the ``.e-fw.json`` extension is only a UX hint): a file with a top-level
    ``models`` key is a workspace file and restores both slots + the active pointer in one action; any
    other valid file is a single-model file and loads into the active slot (cross-format robustness —
    "Open workspace file" fed a single-model file is honest, not an error). Each embedded model is
    recomputed (no calculated attributes in the file); the second model is added through the workspace
    add-to-slot path, so it inherits the shared budget *and* the distinct-system-id invariant (the two
    embedded models may legitimately share an id — e.g. a workspace of a model and its duplicate).
    """
    workspace = SessionWorkspaceRepository(request.session)
    ...
    try:
        ...
        if os.environ.get("RAISE_EXCEPTIONS"):
            raise
        import_error_message += (
            f"Not a valid e-footprint workspace file ! Please use only files generated by e-footprint "
    ...
    return _render_with_error(request, workspace, import_error_message)
