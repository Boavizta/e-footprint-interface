...
def _render_list_membership_section(model_web, params) -> str:
    from model_builder.adapters.forms.form_context_builder import FormContextBuilder

    object_id = params["object_id"]
    web_obj = model_web.get_web_object_from_efootprint_id(object_id)
    context = FormContextBuilder.build_list_membership_section_context(web_obj)
    section_html = render_to_string(
        "model_builder/side_panels/edit/list_membership_section.html", context)
    return f"<div hx-swap-oob='outerHTML:#list-membership-section-{object_id}'>{section_html}</div>"
...
OOB_REGION_RENDERERS: Dict[str, Callable] = {
    ...
    "list_membership_section": _render_list_membership_section,
