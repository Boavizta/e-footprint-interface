...
import pytest
from model_builder.adapters.forms.form_data_parser import parse_form_data, make_form_data_mutable
...
class TestMakeFormDataMutable:
    """Tests for make_form_data_mutable function."""

    def test_returns_copy_of_dict(self):
        """Should return a copy of regular dict."""
        original = {"key": "value"}
        result = make_form_data_mutable(original)

        assert result == original
        assert result is not original

    def test_handles_querydict_like_objects(self):
        """Should call .copy() on objects that have it."""
        class MockQueryDict:
        ...
        mock = MockQueryDict({"key": "value"})
        result = make_form_data_mutable(mock)
