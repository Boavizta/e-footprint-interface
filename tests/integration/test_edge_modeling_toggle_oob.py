...
def test_edge_device_group_create_emits_toggle_region():
    """EdgeDeviceGroupWeb.create_side_effects must call super() so the toggle region propagates."""
    repository = _fresh_repository()
    output = _create(repository, "EdgeDeviceGroup", "Group")
    assert output.model_web.has_edge_objects is True
    assert TOGGLE_REGION in {r.key for r in output.oob_regions}


def test_edge_device_group_edit_without_flip_does_not_emit_toggle_region():
    """EdgeDeviceGroupWeb.edit_side_effects super() chain returns no region when nothing flips."""
    repository = _fresh_repository()
...
def test_edge_device_group_delete_emits_toggle_region():
    """EdgeDeviceGroupWeb.delete_side_effects super() chain emits the toggle region on un-flip."""
    repository = _fresh_repository()
    create_output = _create(repository, "EdgeDeviceGroup", "Group")
    group_id = create_output.created_object_id
